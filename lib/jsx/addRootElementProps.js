import { visit } from "unist-util-visit";

import extractRootElementClassName from "../common/extractRootElementClassName.js";

const addRootElementProps = (ast) => {
  const rootElementClassName = extractRootElementClassName(ast);

  visit(ast, "root", (node) => {
    if (node.type === "root") {
      const childrenElements = node.children.filter(
        (el) => el.type === "element",
      );

      if (childrenElements.length) {
        const rootElement = childrenElements[0];

        const propsString = `{...useBlockProps(${rootElementClassName ? `{ className: '${rootElementClassName}' }` : ""})}`;
        rootElement.properties.props = propsString;
      }
    }
  });
};

export default addRootElementProps;
